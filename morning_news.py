...
def generate_script(world_news, japan_news, stock_data, weather):
    ...
    - マークダウン記号（**、##、--- 等）は一切使わない
    - 自然な話し言葉で書く
    - 原稿は必ず最後まで書き切ること。締めの挨拶まで完全に出力すること（途中で絶対に切らない）
    - 原稿全体の目安は600〜900文字（長すぎず短すぎず、自然な長さに収める）

    「放送構成（この順番で）」
    1. 冠頭挨拶
    2. 世界のニュース2〜3本（1本あたり2〜3文で簡潔に）
    3. 日本の重要なニュース2〜3本（1本あたり2〜3文で簡潔に）
    ...
    for model in GEMINI_MODELS:
                for attempt in range(3):
                                try:
                                    ...
                                    # finish_reason が MAX_TOKENS の場合は出力が途中で打ち切られている
                                                    finish_reason = None
                                                    try:
